...
#
#   sudo DEPLOY_CONFIG=deploy/config.env         deploy/bootstrap.sh   # production
#   sudo DEPLOY_CONFIG=deploy/config.staging.env deploy/bootstrap.sh   # staging
#
# DEPLOY_CONFIG is REQUIRED (issue #146) — there is no silent default environment.
# Configure the target first by copying the matching deploy/config*.env.example.
# Tested on Ubuntu/Debian (apt); Amazon Linux notes are in deploy/README.md.

source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_lib.sh"
load_config
require_root
# Hard blocks BEFORE anything is written (#146): a mis-selected DEPLOY_CONFIG must exit
# non-zero with zero changes, not stand up a second install beside the real one.
...
# One in-place migration on an existing file: an ACTIVE UPSTREAMWX_DATA_DIR (from a
# pre-87e2004 example) overrides everything systemd sets — EnvironmentFile= beats
# Environment= — and pointed staging at prod's data dir (2026-07-20 outage). Comment it out;
# the unit pins the data dir at exec time.
...
# DEPLOY_CONFIG is mandatory (#146), so the loaded config is always a real file. A
# re-bootstrap is typically driven FROM the persisted config itself (uwx-ctl bootstrap sets
# DEPLOY_CONFIG=$DEPLOY_CTL_CONFIG) — `install` aborts on "same file" under set -e and used
# to leave uwx-ctl as the unrendered template (issue #148), so skip the copy in that case.
if [ "$(readlink -f "$DEPLOY_CONFIG")" != "$(readlink -f "$DEPLOY_CTL_CONFIG")" ]; then
    install -o root -g "$DEPLOY_GROUP" -m 0640 "$DEPLOY_CONFIG" "$DEPLOY_CTL_CONFIG"
else
    ok "ctl config IS the loaded config ($DEPLOY_CTL_CONFIG) — leaving in place (#148)"
fi
# Refuse to clobber a DIFFERENT environment's wrapper: /usr/local/bin is shared, so on a
# coexisting staging+prod box each env must set its own DEPLOY_CTL_NAME (#146).
_ctl_path="/usr/local/bin/$DEPLOY_CTL_NAME"
if [ -f "$_ctl_path" ]; then
    _baked="$(sed -n 's/^CTL_CONFIG="\(.*\)".*/\1/p' "$_ctl_path" | head -n1)"
    if [ -n "$_baked" ] && [ "$_baked" != "$DEPLOY_CTL_CONFIG" ] && [ "$_baked" != "__CTL_CONFIG__" ]; then
        die "$_ctl_path already belongs to another environment (config: $_baked).
       Set a distinct DEPLOY_CTL_NAME in this config (e.g. uwx-staging) and re-run bootstrap (#146)."
    fi
fi
render_template "$TPL/uwx-ctl" "$_ctl_path"
chmod 0755 "$_ctl_path"
ok "installed $DEPLOY_CTL_NAME -> $_ctl_path (config: $DEPLOY_CTL_CONFIG)"
...
# Pass the selected config through explicitly — deploy.sh also refuses to run without one (#146).
env DEPLOY_CONFIG="$DEPLOY_CONFIG" "$DEPLOY_DIR/deploy.sh" "$DEPLOY_BRANCH"
