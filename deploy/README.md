...
| Service **does not start after a host reboot** (was fine after the last deploy) | The unit was installed but never boot-`enable`d, so `WantedBy=multi-user.target` never took effect. Fixed: `bootstrap.sh` now enables it and every `deploy.sh` re-asserts it. To fix a running host immediately: `sudo systemctl enable --now upstreamwx-api`. Verify: `systemctl is-enabled upstreamwx-api` → `enabled`. |
